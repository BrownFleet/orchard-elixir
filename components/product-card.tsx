    ...
    <Card className="group hover:shadow-lg transition-all duration-300 border-border/50 h-full flex flex-col">
      <CardHeader className="p-0">
        <div className="relative overflow-hidden rounded-t-lg">
          ...
          {product.stock_quantity === 0 && (
            <Badge className="absolute bottom-3 left-3 bg-red-500 text-white">Out of Stock</Badge>
          )}
        </div>
      </CardHeader>
      ...
      <CardContent className="p-4 space-y-3 flex-1">
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm text-muted-foreground">4.8 (127) {/* Mock rating */}</span>
        </div>

        <div className="space-y-1">
          <div className="text-xl font-bold text-primary">€{product.price_eur}</div>
          <div className="text-sm text-muted-foreground">₹{product.price_inr}</div>
        </div>
      </CardContent>

      <CardFooter className="p-4 pt-0">
        <Button className="w-full group" onClick={handleAddToCart} disabled={product.stock_quantity === 0 || isLoading}>
